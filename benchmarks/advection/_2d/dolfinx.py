"""DOLFINx FEM solver for 2D linear advection equation.

Solves: ∂u/∂t + c⃗·∇u = 0  on [0,1]²

Uses SUPG (Streamline Upwind Petrov-Galerkin) stabilization for
advection-dominated problems. Without stabilization, standard Galerkin
FEM produces spurious oscillations for pure advection.

The SUPG weak form adds artificial diffusion only in the streamline direction:
    ∫(∂u/∂t + c⃗·∇u)·(v + τ·c⃗·∇v) dx = 0

where τ is the stabilization parameter (depends on element size and velocity).

Initial condition: Gaussian blob at (0.3, 0.3)
Velocity field: Constant c⃗ = (0.4, 0.4) (diagonal transport)

Boundary conditions:
    - Inflow (x=0 or y=0 where c⃗·n < 0): u = 0
    - Outflow (x=1 or y=1 where c⃗·n > 0): Natural BC (do nothing)
...
def solve_advection_2d(
    ...
    """Solve 2D advection equation with SUPG stabilization using DOLFINx.
    ...
    # τ = h / (2 * |c|) is a common choice for pure advection
    tau = h / (2.0 * c_mag)

    # Time discretization: backward Euler
    # (u^{n+1} - u^n)/dt + c·∇u^{n+1} = 0
    #
    # Standard Galerkin weak form:
    # ∫ u^{n+1}·v dx + dt·∫ (c·∇u^{n+1})·v dx = ∫ u^n·v dx
    #
    # SUPG adds stabilization term to test function:
    # v_supg = v + τ·(c·∇v)

    # SUPG test function modification
    v_supg = v + tau * ufl.dot(c, ufl.grad(v))

    # Bilinear form: mass + advection with SUPG
    a = (
    ...
    # Inflow boundary condition: u = 0 where c·n < 0
    # For c = (0.5, 0.5), inflow is on x=0 and y=0 boundaries
    def inflow_boundary(x):
        return np.logical_or(np.isclose(x[0], 0.0), np.isclose(x[1], 0.0))

    inflow_dofs = fem.locate_dofs_geometrical(V, inflow_boundary)
    u_bc = fem.Function(V)
    u_bc.x.array[:] = 0.0
    ...
    # Assemble system matrix (constant for linear advection with constant velocity)
    ...
    for step in range(num_time_steps):
        ...
        fem.petsc.apply_lifting(b, [a_compiled], [[bc]])
...
def solve(grid_resolution: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    """
    return solve_advection_2d(grid_resolution=grid_resolution)
