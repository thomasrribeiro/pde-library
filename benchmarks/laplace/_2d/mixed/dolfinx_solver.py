"""DOLFINx FEM solver for 2D Laplace equation with mixed boundary conditions.
...
def solve_laplace_2d_mixed_bc(
    ...
    """Solve 2D Laplace equation with mixed boundary conditions using DOLFINx.

    Solves: ∇²u = 0 on [0,1]²
    with Dirichlet BCs on top/bottom and natural (Neumann) BCs on left/right.
    ...
    # Locate boundary facets for top, left, and right edges
    # Bottom has natural (Neumann) BC - no explicit enforcement needed (insulating)
    def top_boundary(x):
    ...
    # Get DOFs on each Dirichlet boundary
    ...
    # Dirichlet BCs on top, left, and right
    # Bottom is natural (homogeneous Neumann) - automatically satisfied (insulating)
    boundary_conditions = [bc_top, bc_left, bc_right]
    ...
    # Note: For homogeneous Neumann BCs, there's no boundary integral term
    ...
    problem = LinearProblem(
        ...
        petsc_options_prefix="laplace_mixed_",
        petsc_options={
            "ksp_type": "preonly",
            "pc_type": "lu",
        },
...
def solve(grid_resolution: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Returns:
        ...
        - node_positions: shape (N, 2) array of (x, y) coordinates
    """
    return solve_laplace_2d_mixed_bc(grid_resolution=grid_resolution, polynomial_degree=1)
