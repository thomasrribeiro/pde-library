"""2D Laplace Equation Benchmark

Verifies Warp FEM implementation against analytical solution:
    ∇²u = 0  on [0,1]²
...
Exact solution: u(x,y) = sin(πx) · sinh(πy) / sinh(π)

Usage (from project root):
    source .venv/bin/activate && uv run python benchmarks/laplace/_2d/main.py
    source .venv/bin/activate && uv run python benchmarks/laplace/_2d/main.py --resolution 64
    source .venv/bin/activate && uv run python benchmarks/laplace/_2d/main.py --resolution 8 16 32 64 --convergence
    source .venv/bin/activate && uv run python benchmarks/laplace/_2d/main.py --plot
    source .venv/bin/activate && uv run python benchmarks/laplace/_2d/main.py --force  # recompute despite cache
"""
...
from benchmarks.laplace._2d.solver import solve_laplace_2d
...
def run_single_resolution(
    ...
    numerical_solution, node_positions, was_cached = solve_with_cache(
        solver_function=solve_laplace_2d,
    ...
    # Expected max is at top center (0.5, 1.0) = sin(π/2) * sinh(π) / sinh(π) = 1.0
    top_center_idx = np.argmin(np.sum((node_positions - [0.5, 1.0])**2, axis=1))
    print(f"Value at top center (0.5, 1.0): numerical={numerical_solution[top_center_idx]:.6f}, analytical={analytical_solution[top_center_idx]:.6f}")
    ...
    if plot:
        figure = create_solution_comparison_from_points(
            ...
            numerical_solution,
            analytical_solution,
            plot_title=f"2D Laplace Equation - Resolution {resolution}x{resolution}",
        )
        save_figure(figure, RESULTS_DIR / f"laplace_2d_res{resolution:03d}.png")
        show_figure(figure)
...
def run_convergence_study(resolutions: list, force_recompute: bool = False) -> None:
    ...
    if len(results) >= 2:
        ...
        figure = create_convergence_plot(
            ...
            plot_title="2D Laplace Equation - Mesh Convergence",
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="2D Laplace Equation Benchmark",
    ...
    print("=" * 60)
    print("2D LAPLACE EQUATION BENCHMARK")
    print("=" * 60)
    print("Problem: ∇²u = 0 on [0,1]²")
    print("BC:      u(x,1) = sin(πx), u = 0 on other boundaries")
    print("Exact:   u(x,y) = sin(πx) · sinh(πy) / sinh(π)")
