"""Analytical solutions for 3D Poisson equation benchmark.

Manufactured solution: u(x,y,z) = sin(πx)sin(πy)sin(πz)

This satisfies:
    -∇²u = f  where f(x,y,z) = 3π²sin(πx)sin(πy)sin(πz)
...
def compute_analytical_solution(
    ...
    """Compute the exact solution u(x,y,z) = sin(πx)sin(πy)sin(πz).
...
def compute_source_term(
    ...
    """Compute the source term f(x,y,z) = 3π²sin(πx)sin(πy)sin(πz).

    This is derived from -∇²u where u = sin(πx)sin(πy)sin(πz).
