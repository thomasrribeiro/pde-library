"""Warp FEM solver for 3D Poisson equation.
...
def manufactured_source_function(position: wp.vec3):
    ...
    return 3.0 * pi * pi * wp.sin(pi * x) * wp.sin(pi * y) * wp.sin(pi * z)
...
def solve_poisson_3d(
    ...
    """Solve 3D Poisson equation with manufactured solution.
    ...
    # Apply homogeneous Dirichlet BC on all boundary faces
...
def solve(grid_resolution: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    import warp as wp
    wp.init()
    return solve_poisson_3d(grid_resolution=grid_resolution, quiet=True)
